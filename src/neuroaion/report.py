...
def build_markdown(state: ReviewState, prose: dict[str, str]) -> str:
    ...
    _byline = p.byline_full()
    md = f"""# {p.title or 'Systematic Review'}

{('*' + _byline + '*' + chr(10) + chr(10)) if _byline else ''}> Systematic review and meta-analysis conducted and reported in accordance with PRISMA 2020.
...
- **Funding (PRISMA 25).** No specific grant from any funding agency was received for this review.
- **Competing interests (PRISMA 26).** None declared.
- **Data & code availability (PRISMA 27).** All supplementary data are saved alongside
...
def save_locally(out_dir: Path, dest: str | Path) -> Path:
    """Copy the run's bundle to an arbitrary local path (e.g. a Desktop folder).

    When the engine runs on your own machine, point ``dest`` at any directory or
    ``.zip`` path — e.g. ``~/Desktop/my_review.zip`` — and the full review bundle
    is written there.
    """
    import shutil
