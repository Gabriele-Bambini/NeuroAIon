...
1. **Fail-closed.** Never run on mock/placeholder data unless explicitly `--allow-mock`. A missing/invalid LLM key or unreachable sources aborts with a clear error; it never emits a paper.
